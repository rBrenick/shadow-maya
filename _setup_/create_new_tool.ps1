...
# renames all the instances of the current ShadowMaya with the prompted new name
# searches through files and replaces in there as well

# parent directory of the shadow-maya
$TOOLS_DIR = (Get-Item -Path "..\").FullName

# define current tool name variables
$CURRENT_base_case = "shadow-maya"
$CURRENT_snake_case = "shadow_maya"
$CURRENT_PascalCase = "ShadowMaya"
...
# Find instances of shadow_maya in files and replace them with the new tool name
...
Foreach-Object {
    ...
    ((Get-Content -path $_.FullName -Raw) -replace "shadow-maya", $NEW_base_case) | Set-Content -Path $_.FullName
