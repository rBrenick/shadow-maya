...
$TOOL_GIT_REPO = "https://github.com/rBrenick/shadow-maya/archive/main.zip" # modify these after uploading the tool
$UPDATE_PATHS = @("docs", "shadow_maya", "README.md", "setup.py")

$CURRENT_TOOL_FOLDER = (Get-Item -Path ".").FullName
$CURRENT_shadow_maya = (Get-Item -Path ".").Name

$NEW_VERSION_ZIP = $USER_DOWNLOADS_FOLDER + "\" + $CURRENT_shadow_maya + "_update.zip"
$NEW_VERSION_ZIP_FOLDER = $USER_DOWNLOADS_FOLDER + "\" + $CURRENT_shadow_maya + "_update"
...
foreach ($element in $UPDATE_PATHS) {
    ...
    $src_file_path = "$NEW_VERSION_ZIP_FOLDER\$CURRENT_shadow_maya-master\$element"
